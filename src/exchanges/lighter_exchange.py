...
try:
    from lighter import AccountApi, OrderApi
...
class LighterExchange(BaseExchange):
    ...
    def __init__(self, name: str, config: Dict, secrets: Dict):
        ...
        self.api_url = self.config.get('networks', {}).get(self.network_type.value, {}).get('rest_base_url', 'https://api.lighter.xyz')
    ...
    async def create_order(self, symbol: str, order_type: str, side: str, 
                         amount: float, price: Optional[float] = None, params: Dict = {}) -> Dict:
        """Create order"""
        if not self.signer:
        ...
        if order_type == 'market':
             ...
             if price_int == 0:
                  ...
                  if is_ask: price_int = 1
                  else: price_int = 1000000 * (10 ** price_decimals)
    ...
    async def set_leverage(self, symbol: str, leverage: int, margin_mode: str = 'isolated'):
        ...
        mode = SignerClient.ISOLATED_MARGIN_MODE if margin_mode.lower() == 'isolated' else SignerClient.CROSS_MARGIN_MODE
    ...
    async def transfer_funds(self, asset_symbol: str, amount: float, from_type: str, to_type: str):
        ...
        # Determine Route
        route_map = {
            'spot': SignerClient.ROUTE_SPOT,
            'perp': SignerClient.ROUTE_PERP
        }
        ...
        try:
             ...
             if l1_key and l1_key.startswith('0x'): l1_key = l1_key[2:]
    ...
    async def close_position(self, symbol: str):
        ...
        try:
            ...
            if target_pos:
                 ...
                 self.logger.info(f"Closing position for {symbol}: Size {size_float} (Sign {target_pos.sign}) -> Sending Market {'Sell' if close_side_is_ask else 'Buy'}")
    ...
    async def cancel_order(self, symbol: str, order_id: str):
        ...
        try:
            oid = int(order_id)
            # Async call
            res = await self.signer.cancel_order(
                market_index=market_index,
                order_index=oid
            )

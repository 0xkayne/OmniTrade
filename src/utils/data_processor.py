...
class DataProcessor:
    ...
    def calculate_spread_stats(orderbooks: dict[str, dict]) -> dict[str, float]:
        ...
        for _exchange, symbol_data in orderbooks.items():
    ...
    def normalize_orderbook(orderbook: dict, symbol: str) -> dict:
        ...
        if "bids" in orderbook:
            normalized["bids"] = [[float(price), float(quantity)] for price, quantity in orderbook["bids"]]
        else:
            normalized["bids"] = []

        # 处理asks
        if "asks" in orderbook:
            normalized["asks"] = [[float(price), float(quantity)] for price, quantity in orderbook["asks"]]
        else:
            normalized["asks"] = []
    ...
    @staticmethod
    def calculate_weighted_average_price(orderbook: dict, depth: int = 5) -> float | None:
        """计算加权平均价格"""
        if not orderbook.get("bids") or not orderbook.get("asks"):
            return None
        ...
        weighted_bid = (
            sum(price * qty for price, qty in zip(bid_prices, bid_quantities, strict=True)) / total_bid_quantity
        )
        ...
        weighted_ask = (
            sum(price * qty for price, qty in zip(ask_prices, ask_quantities, strict=True)) / total_ask_quantity
        )
    ...
    def _merge_volume_weighted(orderbooks: list[dict], symbol: str) -> dict:
        ...
        # 转换回列表并排序
        merged_bids = sorted([[price, qty] for price, qty in bid_dict.items()], key=lambda x: x[0], reverse=True)
        merged_asks = sorted([[price, qty] for price, qty in ask_dict.items()], key=lambda x: x[0])

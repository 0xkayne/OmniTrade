...
class ArbitrageOpportunity:
    ...
    def __str__(self):
        return (
            f"Arbitrage[{self.symbol}]: {self.buy_exchange}@{self.buy_price:.2f} -> "
...
class SpreadArbitrageStrategy:
    ...
    def _calculate_directional_opportunity(
        self, symbol: str, buy_exchange: str, sell_exchange: str, buy_orderbook: dict, sell_orderbook: dict
    ...
    def should_execute_trade(self, opportunity: ArbitrageOpportunity, balances: dict[str, dict]) -> bool:
        """判断是否应该执行交易"""
        # 检查价差是否足够
        ...
        if buy_balance.get("free", 0) < required_buy_amount:
            self.logger.warning(
                f"资金不足: {opportunity.buy_exchange} "
        ...
        # 检查卖出资产是否足够
        sell_balance = balances.get(opportunity.sell_exchange, {}).get(opportunity.symbol.split("/")[0], {})

        if sell_balance.get("free", 0) < opportunity.volume:
            self.logger.warning(
                f"资产不足: {opportunity.sell_exchange} "
    ...
    def calculate_position_size(
        self, opportunity: ArbitrageOpportunity, total_capital: float, risk_per_trade: float = 0.02
    ) -> float:
        """根据资金管理计算仓位大小"""

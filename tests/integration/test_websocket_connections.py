...
class TestWebSocketIntegration:
    ...
    async def test_multiple_exchange_websocket_connections(self, sample_config, sample_secrets):
        ...
        # 创建多个交易所配置
        exchanges_config = {
            "lighter": {**sample_config, "type": "native"},
            "hyperliquid": {
                **sample_config,
                "type": "ccxt",
                "networks": {
        ...
        secrets = {"lighter": sample_secrets, "hyperliquid": sample_secrets}
        ...
        with patch("websockets.connect") as mock_ws:
            ...
            # 测试WebSocket连接
            for _name, exchange in exchanges.items():
    ...
    async def test_websocket_subscription_integration(self, sample_config, sample_secrets):
        ...
        with patch("websockets.connect") as mock_ws:
            ...
            # 验证订阅消息发送
            expected_subscribe = {"action": "subscribe", "channel": "orderbook", "symbol": "ETHUSD"}
